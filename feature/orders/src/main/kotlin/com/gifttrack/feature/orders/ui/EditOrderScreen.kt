...
import com.gifttrack.feature.orders.viewmodel.EditOrderViewModel

/**
 * Screen for editing an existing order.
 *
 * @param onNavigateBack Callback when user navigates back.
 * @param viewModel ViewModel for managing edit order state.
 */
...
fun EditOrderScreen(
    ...
    LaunchedEffect(uiState) {
        when (uiState) {
            is EditOrderUiState.Success -> {
                snackbarHostState.showSnackbar("Änderungen erfolgreich gespeichert")
                onNavigateBack()
            }
            is EditOrderUiState.Error -> {
                val errorMessage = (uiState as EditOrderUiState.Error).message
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Bestellung bearbeiten") },
                navigationIcon = {
        ...
        Box(
            ...
            when (uiState) {
                is EditOrderUiState.Loading -> {
                    CircularProgressIndicator(
                        modifier = Modifier.align(Alignment.Center)
                    )
                }
                is EditOrderUiState.Saving -> {
                    CircularProgressIndicator(
                        modifier = Modifier.align(Alignment.Center)
                    )
                }
                is EditOrderUiState.Editing, is EditOrderUiState.Error -> {
                    EditOrderForm(
                        formState = formState,
...
/**
 * Form content for editing an order.
 */
@Composable
